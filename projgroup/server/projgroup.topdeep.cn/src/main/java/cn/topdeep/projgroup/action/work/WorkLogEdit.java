...
public class WorkLogEdit extends BaseAction {
	...
	public void validateSave() throws DbException, SQLException, CacheException {
		...
		condition.or().andWorkUserIdEqualTo(getLoginUserId()).andWorkDayEqualTo(getWorkLog().getWorkDay());
		List<WorkLog> logList = getSystemBiz().workLogSelectObjectsWithBLOBs(condition);
		if(logList.size() > 0){
			WorkLog qWorkLog = logList.get(0);
			if(qWorkLog.getWorkLogId().intValue() != getWorkLog().getWorkLogId().intValue()){
