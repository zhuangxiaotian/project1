...
public class CodeBuildProcedureList extends BaseAction {
			...
			for (FuncProcedure item : list) {
				if (!procSet.contains(item.getFuncProcedureId())) {
					if (oldProcSet.contains(item.getFuncProcedureId())) {
						...
						condition.or().andBuildIdEqualTo(buildId).andFuncProcedureIdEqualTo(item.getFuncProcedureId());
								...
								if (one.getFuncProcedureId() == item.getFuncProcedureId()) {
									...
									for (TestParamValue delete : deleteparam) {
										if (delete.getTestProcedureId() == one.getTestProcedureId()) {
											funcProjectBiz.testParamValueDeleteByParamIdTestProcedureId(
													delete.getParamId(), delete.getTestProcedureId());
										}
									}
	...
	private String invertImportFrom() throws DbException, SQLException, CacheException {
		if (!hasPrivilege(Privilege.FUNC_FUNC_BUILD_MANAGE, PrivilegeOperate.Write)) {
			return NO_PRIVILEGES;
		}
	...
	public String importFrom() throws DbException, SQLException, CacheException {
		if (!hasPrivilege(Privilege.FUNC_FUNC_BUILD_MANAGE, PrivilegeOperate.Write)) {
			return NO_PRIVILEGES;
		}
		if (invertImport == 1) {
	...
	/**
	 * @param funcProjectBiz
	 *            the funcProjectBiz to set
	 */
	...
	/**
	 * @param selectedFuncIds
	 *            the selectedFuncIds to set
	 */
	 ...
	 */
	public List<FuncProcedure> getProcList() throws DbException, SQLException, CacheException {
		if (procList == null && buildId > 0) {
			CodeBuild build = funcProjectBiz.codeBuildSelectByPrimaryKey(buildId);
			if (build != null && !StringUtils.isNullOrEmpty(build.getFuncProjects())) {
				...
				procList = funcProjectBiz.funcProcedureSelectObjects(condition);
