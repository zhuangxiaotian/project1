...
public class FuncImport extends BaseAction {
	...
	private File upload;
	private String importFuncIds;
	private String adjustFuncPrefix;
	...
	private FuncParams readParam(FuncProcedure func, Map<String, String> dataMap) {
		...
		param.setPub0("");
		param.setPub1(func.getProcVersion());
		return param;
	}
	...
	private void importTopdeepData(List<Map<String, String>> data) throws Exception {
		...
		condition1.or().andFuncProjectIdEqualTo(funcProjectId);
		condition1.setPage(new PageImpl(0, 1));
		condition1.setOrderByClause("ORDER_FIELD desc");
		FuncProcedure maxProcedure = funcProjectBiz.selectSingleObject(funcProjectBiz.funcProcedureSelectObjects(condition1));
		int currentOrder = 0;
		if (maxProcedure != null) {
			currentOrder = maxProcedure.getOrderField() + 1;
		}
			...
			try {
				...
				if ("功能".equals(lastCategory)) {
					...
					if (func != null) {
						...
						condition.or().andFuncCodeEqualTo(func.getFuncCode()).andProcVersionEqualTo(func.getProcVersion())
								.andProcCodeEqualTo(func.getProcCode());
						FuncProcedure oldFunc = systemBiz.selectSingleObject(systemBiz.funcProcedureSelectObjects(condition));
						if (oldFunc != null) {
							func.setOrderField(oldFunc.getOrderField());
							...
							func.setOrderField(currentOrder++);
				...
				if ("调用公共包".equals(lastCategory)) {
					...
					if (param != null) {
						param.setOrderField(paramOrder++);
	...
	public String importFuncs() {
		message = "开始导入";
		if (importFuncIds != null) {
			String[] strIds = importFuncIds.split(",");
			for (int i = 0; i < strIds.length; i++) {
				int pid = Integer.parseInt(strIds[i]);
				try {
					importFunc(pid);
					message += "导入" + pid + "完成;";
				} catch (Exception e) {
	...
	private static String funcLetters = "0123456789abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ";

	private String getNextFuncCode(String currentFuncCode) throws Exception {
		if (currentFuncCode.length() != 2) {
			throw new Exception("无效的初始函数代码");
		}
		String lettle1 = currentFuncCode.substring(0, 1);
		String lettle2 = currentFuncCode.substring(1, 2);
		boolean addLettle1 = false;
		int lettle1Pos = funcLetters.indexOf(lettle1);
		int lettle2Pos = funcLetters.indexOf(lettle2);
		if (lettle1Pos < 0) {
			throw new Exception("无效的初始函数代码");
		}
		if (lettle2Pos < 0) {
		...
		if (lettle2Pos >= funcLetters.length() - 1) {
			addLettle1 = true;
		}
		if (addLettle1) {
			lettle1 = funcLetters.substring(lettle1Pos + 1, lettle1Pos + 2);
			lettle2 = funcLetters.substring(0, 1);
		} else {
			lettle2 = funcLetters.substring(lettle2Pos + 1, lettle2Pos + 2);
		}
		return lettle1 + lettle2;
	}

	public String adjustFuncCodes() {
		if (StringUtils.isNullOrEmpty(adjustFuncPrefix)) {
			message = "参数错误 x|y:z   x,y是要整理的功能号首字母,z是整理后的首字母";
			return SUCCESS;
		}
		log.debug("adjustFuncPrefix=" + adjustFuncPrefix);
		String[] p1s = adjustFuncPrefix.split(":");
		String targetPrefix = p1s[1];
		log.debug("targetPrefix=" + targetPrefix);
		String[] p2s = p1s[0].split("\\|");
		HashSet<String> prefixSet = new HashSet<String>();
		for (String item : p2s) {
			if (!prefixSet.contains(item)) {
				prefixSet.add(item);
				log.debug("add adjust prefix: " + item);
			}
		}
		try {
			FuncProcedureExample condition = new FuncProcedureExample();
			condition.or().andFuncProjectIdEqualTo(funcProjectId);
			condition.setOrderByClause("FUNC_PROCEDURE_ID asc");
			List<FuncProcedure> procList = funcProjectBiz.funcProcedureSelectObjects(condition);
			String currFunc = "00";
			for (FuncProcedure item : procList) {
				if (!prefixSet.contains(item.getFuncCode().substring(0, 1))) {
					continue;
				}
				currFunc = getNextFuncCode(currFunc);
				item.setFuncCode(targetPrefix + currFunc);
				funcProjectBiz.funcProcedureUpdate(item);
			}
		} catch (Exception ex) {
	...
	private void importFunc(int funcId) throws DbException, SQLException, CacheException {
		FuncProcedure proc = funcProjectBiz.funcProcedureSelectByPrimaryKey(funcId);
		FuncProcedure newProc = new FuncProcedure();
		newProc.setCacheTime(proc.getCacheTime());
		newProc.setFuncCategory(proc.getFuncCategory());
		newProc.setFuncCode(proc.getFuncCode());
		newProc.setFuncInputInterface(proc.getFuncInputInterface());
		newProc.setFuncOutputInterface(proc.getFuncOutputInterface());
		newProc.setFuncProjectId(funcProjectId);
		newProc.setOrderField(proc.getOrderField());
		newProc.setProcCode(proc.getProcCode());
		newProc.setProcComment(proc.getProcComment());
		newProc.setProcName(proc.getProcName());
		newProc.setProcType(proc.getProcType());
		newProc.setProcVersion(proc.getProcVersion());
		newProc.setPub0(proc.getPub0());
		newProc.setPub1(proc.getPub1());
		newProc.setPub2(proc.getPub2());
		newProc.setPub3(proc.getPub3());
		newProc.setPub4(proc.getPub4());
		newProc.setPub5(proc.getPub5());
		funcProjectBiz.beginTransaction();
		try {
			funcProjectBiz.funcProcedureInsert(newProc);
			List<FuncParams> paramList = funcProjectBiz.funcParamsSelectObjectsByFuncProcedureId(proc.getFuncProcedureId());
			Hashtable<Integer, Integer> idMap = new Hashtable<Integer, Integer>();
			for (FuncParams item : paramList) {
				FuncParams newItem = new FuncParams();
				newItem.setFuncProcedureId(newProc.getFuncProcedureId());
				newItem.setImplInterface(item.getImplInterface());
				newItem.setOrderField(item.getOrderField());
				newItem.setParamAllowModify(item.getParamAllowModify());
				newItem.setParamCategory(item.getParamCategory());
				newItem.setParamCode(item.getParamCode());
				newItem.setParamComment(item.getParamComment());
				newItem.setParamDataDecimal(item.getParamDataDecimal());
				newItem.setParamDataDefaultValue(item.getParamDataDefaultValue());
				newItem.setParamDataLength(item.getParamDataLength());
				newItem.setParamDataNotNull(item.getParamDataNotNull());
				newItem.setParamDataType(item.getParamDataType());
				newItem.setParamDirection(item.getParamDirection());
				// newItem.setParamId(value)
				newItem.setParamName(item.getParamName());
				newItem.setPub0(item.getPub0());
				newItem.setPub1(item.getPub1());
				newItem.setPub2(item.getPub2());
				newItem.setPub3(item.getPub3());
				newItem.setPub4(item.getPub4());
				newItem.setPub5(item.getPub5());
				if (idMap.containsKey(item.getParentParamId())) {
					newItem.setParentParamId(idMap.get(item.getParentParamId()));
				} else {
					newItem.setParentParamId(0);
				}
				funcProjectBiz.funcParamsInsert(newItem);
				idMap.put(item.getParamId(), newItem.getParamId());
			}
	...
	/**
	 * @return the importFuncIds
	 */
	public String getImportFuncIds() {
		return importFuncIds;
	}

	/**
	 * @param importFuncIds the importFuncIds to set
	 */
	public void setImportFuncIds(String importFuncIds) {
		this.importFuncIds = importFuncIds;
	}

	/**
	 * @return the adjustFuncPrefix
	 */
	public String getAdjustFuncPrefix() {
		return adjustFuncPrefix;
	}

	/**
	 * @param adjustFuncPrefix the adjustFuncPrefix to set
	 */
	public void setAdjustFuncPrefix(String adjustFuncPrefix) {
		this.adjustFuncPrefix = adjustFuncPrefix;
	}
}
